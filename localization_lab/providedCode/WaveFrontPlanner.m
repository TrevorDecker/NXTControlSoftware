%% Wave Front Planner
%% Calculates the shortest path from the start cell to the end cell with
%% a wave front planner and plots the path on the current gird
...
% StaffTODO:	 Detect No path from Finish to Start and throw an exception
%			 Move away from global Variables

function [ plan ] = wavefrontPlanner(map,start,finish)
%NOTE mapGenerator must be run before this funciton
global DX
global DY
assert(DX >0)
assert(DY >0)
...
%STAFF_TODO detect No path from Finish to Start and throw an exception 
...
function plotPath(path)
    global DX
    global DY
    %draws path in the center of each grid cell that will be passed through
     hold on
